...
satisfiable (Body ts) = eval ts
satisfiable (Exists item evaluation) =  
  elem True ((satisfiable . evaluation . Con) <$> item)
...
solutions (Body f) 
  | eval f = [()]
  | otherwise = []
solutions (Exists item evaluation) = [
  (i, j)
  | i <- filter (satisfiable . evaluation . Con) item ,
    j <- (solutions . evaluation . Con) i
  ]
